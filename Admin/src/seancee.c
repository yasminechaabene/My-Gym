...
void afficher_seancee(GtkWidget *liste)	
{
	...
	{ f = fopen("seancee.txt", "a+");
              while(fscanf(f,"%s %s %s %s %s %s %s %s\n",seancee1.num,seancee1.cours,seancee1.date.jour,seancee1.date.mois,seancee1.date.annee,seancee1.heure,seancee1.nplace,seancee1.nplacemax)!=EOF)
		{
	strcpy(ch,seancee1.date.jour);strcat(ch,"/");
	strcat(ch,seancee1.date.mois);strcat(ch,"/");
	strcat(ch,seancee1.date.annee);
	gtk_list_store_append (store, &iter);
	gtk_list_store_set (store, &iter, NUM, seancee1.num, COURS, seancee1.cours, DATE, ch,HEURE,seancee1.heure,NPLACE,seancee1.nplace, -1); 
		}
		fclose(f);
...
int modif_nb2(char num[]) 
{
    ...
    if (f!=NULL)
    {
        while(!feof(f))
        {
            fscanf(f,"%s %s %s %s %s %s %s %s\n",seancee1.num,seancee1.cours,seancee1.date.jour,seancee1.date.mois,seancee1.date.annee,seancee1.heure,seancee1.nplace,seancee1.nplacemax);
