...
int modif_nb4(char num[]) 
{
    ...
    f1=NULL;
    f=fopen("seances_cures.txt","r+");
    f1=fopen("seances_cures.txt.tmp","a+");
    if (f!=NULL)
    {
        while(!feof(f))
        {
    ...
    fclose(f1);
    fclose(f);
    rename("seances_cures.txt.tmp","seances_cures.txt");
...
rename("seancec2.txt","seances_cures1.txt");
